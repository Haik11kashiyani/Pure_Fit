...
if (!function_exists('purefit_base_url')) {
    function purefit_base_url(): string
    {
        $host = $_SERVER['HTTP_HOST'] ?? 'localhost';
        $isHttps = !empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off';
        $scheme = $isHttps ? 'https' : 'http';
        $requestUri = $_SERVER['REQUEST_URI'] ?? '/';
        $dir = rtrim(str_replace('\\', '/', dirname($requestUri)), '/');
        if ($dir === '.' || $dir === '/') {
            $dir = '';
        }
...
function send_verification_email_smtp($email, $token) {
    ...
    try {
        ...
        $baseUrl = purefit_base_url();
        $verify_url = rtrim($baseUrl, '/') . '/verify.php?token=' . $token;
    ...
    try {
        ...
        <body>
            <div class="container">
                ...
                <div class="button-container">
                    <a href="' . rtrim(purefit_base_url(), '/') . '/login.php" class="button">Login to Your Account</a>
                </div>
                <div class="content">
        ...
        $mail->AltBody = 'Welcome to Pure Fit! Your account is now active. Login here: ' . rtrim(purefit_base_url(), '/') . '/login.php';
